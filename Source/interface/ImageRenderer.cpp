...
void ImageRenderer::init(OpenGLWrapper& opengl) {
    uwu::glTools::genBuffers(opengl, vao, vbo, ibo);
    uwu::glTools::setBufferSizes<vertex>(opengl, vbo, ibo, 4, 6);
}
void ImageRenderer::setImage(OpenGLWrapper& opengl, const char* file, float _x, float _y) {
    if (_x != -1) setPos(_x, _y);

    texture = opengl.getTexture(file);
}void ImageRenderer::setImage(OpenGLWrapper& opengl, juce::OpenGLTexture* _texture, float _x, float _y) {
    if (_x != -1) setPos(_x, _y);

    texture = _texture;
}
void ImageRenderer::update(OpenGLWrapper& opengl) {
    ...
    texture->bind();
    opengl.tex.shaderProgram->use();
    uwu::glTools::draw(opengl, vao, idx.size());
    texture->unbind();
}
void ImageRenderer::destroy(OpenGLWrapper& opengl) {
